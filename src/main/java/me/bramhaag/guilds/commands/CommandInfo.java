...
public class CommandInfo extends CommandBase {
    ...
    public void execute(Player player, String[] args) {
        ...
        Message.sendMessage(player, Message.COMMAND_INFO_MEMBER_COUNT.replace("{members}", String.valueOf(guild.getMembers().size()), "{members-online}", String.valueOf(guild.getMembers().stream().map((member) -> Bukkit.getPlayer(member.getUniqueId())).filter(OfflinePlayer::isOnline).count())));
