...
public class LeaderboardListDeserializer implements JsonDeserializer<List<Leaderboard>> {
    ...
    @Override
    public List<Leaderboard> deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
        ...
        object.forEach(obj -> {
            ...
            leaderboards.add(new Leaderboard(leaderboard.get("name").getAsString(), Leaderboard.LeaderboardType.valueOf(leaderboard.get("leaderboardType").getAsString()), Leaderboard.SortType.valueOf(leaderboard.get("sortType").getAsString()), scores) {});
        });
